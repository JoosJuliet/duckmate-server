...
request({
        ...
        for(var i=0;i<100;i++){
                ...
                Artistarr[i] = Artistarr[i].replace(/\'/g, "\\\'");
                Songarr[i] = Songarr[i].replace(/\n/g, '');
                Songarr[i] = Songarr[i].replace(/\s/g, '');
                Songarr[i] = Songarr[i].replace(/\'/g, "\\\'");
        }
        var num = 1;
...
fs.readFile('/home/node/duckmate-server/crawling/bugs_title.txt', function(err, data){
        if(err) throw err;
        var array = data.toString().split("\r");
        var query = "";
        var num = 1;
        ...
        for(i in array){
                query += "update chart_bugs set song = '"+ array[i] +"' where idx='" + num + "'; ";
        num=num+1;
        }
        pool.query(query, function(error, rows){
                if(error){
